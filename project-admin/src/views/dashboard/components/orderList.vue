<template>
  <div>
    <div class="container homecon">
      ...
      <div class="">
        <div v-if="orderData.length > 0">
          ...
          >
            <el-table-column prop="number" label="订单号" width="134px" />
            <el-table-column label="订单菜品" width="131px">
            ...
            >
              <!-- <template slot-scope="scope">
                <div class="ellipsisHidden">
                  ...
                  >
                    <span slot="reference">{{ row.type===0?'堂食' : '打包' }}</span>
                  </el-popover>
                </div>
              </template> -->
            </el-table-column>
            ...
            <el-table-column
              ...
              width="200"
            />
            <el-table-column prop="amount" label="实收金额(元)" width="131px" />
            <el-table-column label="备注" width="162px">
              <template slot-scope="scope">
                <div class="ellipsisHidden">
                  <el-popover
                    placement="top-start"
            ...
            <el-table-column
              ...
              :min-width="
                [2,4].includes(dialogOrderStatus)
                  ? 130
    ...
    >
        ...
        <div class="order-top">
          <div>
            ...
            <div
              ...
              class="order-status"
              :class="{ status3: [4].includes(dialogOrderStatus) }"
            >
              {{
        ...
        <div class="order-middle">
          <div class="user-info">
            <div class="user-info-box">
              ...
              <div
                v-if="[2, 3, 4].includes(dialogOrderStatus)"
                class="user-getTime"
              >
                <label>{{
                  dialogOrderStatus === 4 ? '完成时间：' : '预计完成时间：'
                }}</label>
                <span>{{
                  dialogOrderStatus === 4
              ...
              <div class="user-address">
                <label>类型：</label>
                <span>{{ diaForm.diningType===0? '堂食' : '打包' }}</span>
              </div>
            </div>
            <div
              class="user-remark"
              :class="{ orderCancel: dialogOrderStatus === 5 }"
            >
              <div>{{ dialogOrderStatus === 5 ? '取消原因' : '备注' }}</div>
              <span>{{
                dialogOrderStatus === 5
            ...
            <div class="amount-list">
              <div class="dish-amount">
                ...
                <span class="amount-price">￥{{
                  ((diaForm.amount - diaForm.packAmount).toFixed(2) *
                    100) /
                    100
                }}</span>
              </div>
      ...
      <span v-if="dialogOrderStatus !== 5" slot="footer" class="dialog-footer">
        <el-checkbox
        ...
        <el-button
          v-if="[1, 4, 5].includes(dialogOrderStatus)"
        ...
        >完 成</el-button>
        <el-button
          v-if="[1,3].includes(dialogOrderStatus)"
          type="primary"
          @click="cancelOrder(row, $event)"
        >取消订单</el-button>
      </span>
...
export default class extends Vue {
  ...
  //获取订单类型
  displayOrderType(row) {
      return row.diningType === 0 ? '堂食' : '打包';
    }
  // // 获取订单数据
